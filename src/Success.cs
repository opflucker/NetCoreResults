...
public sealed class Success<TError> : Result<TError>
{
    ...
    public override T On<T>(Func<T> successFunc, Func<TError, T> failureFunc) => successFunc();
}
...
public sealed class Success<TData, TError> : Result<TData, TError>
{
    ...
    public override T On<T>(Func<TData, T> successFunc, Func<TError, T> failureFunc) => successFunc(data);
}
